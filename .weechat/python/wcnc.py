...
def notify(data, buffer, date, tags, displayed, highlight, prefix, message):
    ...
    if prefix == own_nick or prefix == ('@%s' % own_nick) or prefix == '@root':
    ...
    sound = (weechat.config_get_plugin('sound_name')
             if weechat.config_get_plugin('sound') == 'on'
             else lambda:_)
    activate_bundle_id = weechat.config_get_plugin('activate_bundle_id')
    pync.notify(message, title='%s [private]' % prefix, sound=sound,
